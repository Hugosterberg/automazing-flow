...
export const DEFAULT_OAUTH_ERROR_MESSAGES: Record<string, string> = {
  ...
  judgeme_not_configured:
    "Judge.me saknar shop-domän eller API-token. Koppla via dialogen (shop-domän + privat API-token) eller lägg till JUDGEME_SHOP_DOMAIN och JUDGEME_API_TOKEN under Inställningar → API-nycklar.",
  judgeme_rejected:
    "Judge.me avvisade uppgifterna. Kontrollera shop-domänen och den privata API-token i Judge.me admin → Settings → Integrations.",
