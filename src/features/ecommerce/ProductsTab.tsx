...
function ProductThumb({
  ...
  return (
    <img
      src={src}
      alt={image?.alt || fallbackLabel || t("products.productImageAlt")}
...
const SOURCE_KEYS: Product["source"][] = ["manual", "alibaba", "shopify", "content"];
...
export function ProductsTab({
  ...
  const { t } = useTranslation("ecommerce");
  const sourceLabel = (source: Product["source"]) => t(`sources.${source}`);
  ...
  const sourceChips = useMemo(() => {
    return SOURCE_KEYS.filter((source) => (sourceCounts[source] ?? 0) > 0);
  }, [sourceCounts]);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : t("toasts.productCreateFailed"));
    } finally {
      setCreating(false);
    }
  }
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {products.length === 0
              ...
              : t("products.countInCatalog", { count: products.length })}
          </p>
        </div>
      ...
      {products.length > 0 ? (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              <option value="updated">{t("products.sortUpdated")}</option>
      ...
      {loading && products.length === 0 ? (
        ...
        <Card className="bg-card border-border border-dashed">
          <CardContent className="py-10 text-center space-y-3">
            <p className="text-sm text-muted-foreground">{t("products.noMatch")}</p>
            <Button
              variant="outline"
              size="sm"
              onClick={() => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {visibleProducts.map((product) => {
            ...
            const meta = [product.versions.length > 0 ? t("products.versionCount", { count: product.versions.length }) : null]
              .filter(Boolean)
              .join(" · ");
            return (
              <Card
      ...
      <Dialog open={newOpen} onOpenChange={(open) => !creating && setNewOpen(open)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("products.dialog.newTitle")}</DialogTitle>
            <DialogDescription>{t("products.dialog.newDescription")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-1">
            <div className="space-y-1.5">
              <Label htmlFor="new-product-name">{t("products.dialog.nameLabel")}</Label>
              <Input
                id="new-product-name"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="new-product-description">{t("products.dialog.descriptionLabel")}</Label>
              <Textarea
...
function ProductEditDialog({
  ...
  function addContentImage(asset: SelectedContentAsset) {
    ...
    if (draft.images.some((img) => img.url === url)) {
      toast.info(t("toasts.imageAlreadyOnProduct"));
      return;
    }
  ...
  async function save() {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : t("toasts.productSaveFailed"));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : t("toasts.productDeleteFailed"));
    } finally {
      setSaving(false);
    }
  }
    ...
    <Dialog open onOpenChange={(open) => !open && !saving && onClose()}>
      <DialogContent className="sm:max-w-2xl max-h-[88dvh] overflow-y-auto">
        ...
        <div className="space-y-5 py-1">
          <div className="space-y-1.5">
            <Label htmlFor="edit-product-name">{t("products.dialog.nameLabel")}</Label>
            <Input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="edit-product-price">{t("products.dialog.priceLabel")}</Label>
              <Input
            ...
            <div className="space-y-1.5">
              <Label htmlFor="edit-product-currency">{t("products.dialog.currencyLabel")}</Label>
              <Input
                id="edit-product-currency"
                placeholder={t("products.dialog.currencyPlaceholder")}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="edit-product-description">{t("products.dialog.descriptionEditLabel")}</Label>
            <Textarea
              id="edit-product-description"
              className="min-h-[120px]"
              placeholder={t("products.dialog.descriptionEditPlaceholder")}
          ...
          {(draft.adminUrl || draft.sourceUrl) ? (
            ...
            >
              {draft.source === "shopify" ? t("products.dialog.openInShopify") : t("products.dialog.sourceLink")}
          ...
          {draft.specs.length > 0 ? (
            <div className="space-y-2">
              <p className="text-sm font-medium">{t("products.dialog.specsTitle")}</p>
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium">{t("products.dialog.imagesTitle")}</p>
            {draft.images.length > 0 ? (
                ...
                {draft.images.map((image) => (
                  <div key={image.id} className="relative rounded-md overflow-hidden border border-border group/img">
                    ...
                    <button
                      ...
                      aria-label={t("products.dialog.removeImageAria")}
                    >
              ...
              <p className="text-xs text-muted-foreground">{t("products.dialog.noImages")}</p>
            )}
          </div>
          ...
          <div className="space-y-3 rounded-lg border border-border/70 bg-muted/20 p-4">
            <div>
              <p className="text-sm font-medium flex items-center gap-2">
                ...
                {t("products.dialog.versionsTitle")}
              </p>
              <p className="text-xs text-muted-foreground">
                {t("products.dialog.versionsHint")}
              </p>
            </div>
                  ...
                  <div key={version.id} className="rounded-md border border-border overflow-hidden bg-background group/ver">
                    <div className="relative">
                      ...
                      <button
                        ...
                        aria-label={t("products.dialog.removeVersionAria")}
                      >
                        <X className="h-3.5 w-3.5" />
                      </button>
                    </div>
            ...
            <div className="space-y-2">
              <Input
                placeholder={t("products.dialog.versionNamePlaceholder")}
                value={versionName}
              ...
              {contentAssets.length > 0 ? (
                <div className="space-y-1.5">
                  <p className="text-[11px] uppercase tracking-wide text-muted-foreground flex items-center gap-1">
                    ...
                    {t("products.dialog.contentImagesHint")}
                  </p>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {versionAssetId ? (
                  ...
                  >
                    ...
                    {t("products.dialog.addTaggedToGallery")}
                  </Button>
                ) : null}
              </div>
            </div>
