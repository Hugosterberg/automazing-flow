/**
 * Cron job: emails a gentle "we miss you" win-back nudge to Shopify customers
 * who have gone quiet (deduped with a cooldown, see flowAutomationJobs.ts).
 */

import { automationSettingsRowToDomain } from "../../automation/autoReply.ts";
import { isEmailConfigured } from "../../lib/email.ts";
import { runCustomerWinback } from "../../lib/flowAutomationJobs.ts";

export function registerCustomerWinbackCron(
  app,
  {
    withRunRecording,
    isAuthorisedCron,
  ...
  app.get(
    "/api/cron/customer-winback",
    withRunRecording("customer-winback", supabaseAdmin, async (req, res) => {
      ...
      try {
        ...
        for (const [bpId, row] of settingsByProfile) {
          ...
          if (!settings.jobSchedules["customer-winback"]?.enabled) continue;
          if (!isProfileJobDueNow(row, "customer-winback", cronNow)) continue;
          eligible.push({ bpId });
        }
        ...
        for (const entry of eligible) {
          ...
          try {
            ...
            const result = await runCustomerWinback({
              supabaseAdmin,
            ...
            failed += 1;
            console.warn(
              `[cron] customer-winback bp=${entry.bpId} failed:`,
        ...
        console.error("[cron] customer-winback unexpected:", e?.message || e);
        return res.status(500).json({ error: "customer_winback_failed" });
      }
    })
  );
}
