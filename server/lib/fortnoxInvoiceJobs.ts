...
import { createFortnoxCustomer, createFortnoxInvoice } from "../providers/fortnox.ts";
import { fetchShopifyFulfilledOrders } from "../providers/shopify.ts";
...
export async function createQueuedFortnoxInvoice(deps: {
  ...
  const invoiceResult = await createFortnoxInvoice(fortnoxAccessToken, {
    ...
    rows: item.lineItems.map((li) => ({ description: li.title, price: li.price, quantity: li.quantity })),
  });
